...
class Dtbarangkeluar{
	...
	function get_all_trx($id){
		$sql=mysql_query("select qty from $this->barangkeluaritem where id='$id'");
	...
	function get_total($id,$m,$h){
		$sql=mysql_query("select sum(dtbarangkeluaritem.qty) as jumlah from dtbarangkeluar,dtbarangkeluaritem where dtbarangkeluar.id=dtbarangkeluaritem.idt and dtbarangkeluaritem.id='$id' and dtbarangkeluar.tanggal between '$m' and '$h' order by dtbarangkeluar.tanggal");
		while($data=mysql_fetch_row($sql))
		return $data[0];
	}
	function get_total_trx($id){
		$sql=mysql_query("select sum(qty) as jumlah from dtbarangkeluaritem where id='$id'");
		while($data=mysql_fetch_row($sql))
		return $data[0];
	}
