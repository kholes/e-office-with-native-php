...
$cari=$_GET['cari'];
if(isset($cari)){
	$statement = "dtmail_in where  mto='$logid' and mail_from like '%$cari%' or mail_about like '%$cari%' or mail_date like '%$cari%'";
}else{
	$statement = "dtmail_in where  mto='$logid'";
}
		...
		<div id="head-mail">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				  <th width="18%" align="left" style="padding:5px;"><p style="padding-left:10px">
					<a href="<?php echo $link; ?>&m=min&s=mfrom&i=<?php echo $i;?>&sts=<?php echo $sts;?>&cari=<?php echo $cari;?>">DARI</a></p>				
					</th>
				    <th width="17%" align="left" style="padding:5px;">
					<a href="<?php echo $link; ?>&m=min&s=tanggal&i=<?php echo $i;?>&sts=<?php echo $sts;?>&cari=<?php echo $cari;?>">TANGGAL SURAT</a>				</th>
				<th width="28%" align="left">
					<a href="<?php echo $link; ?>&m=min&s=mail_from&i=<?php echo $i;?>&sts=<?php echo $sts;?>&cari=<?php echo $cari;?>">SURAT DARI</a>				</th>
				<th width="30%" align="left" style="padding:5px;">
					<a href="<?php echo $link; ?>&m=min&s=mail_about&i=<?php echo $i;?>&sts=<?php echo $sts;?>&cari=<?php echo $cari;?>">URAIAN SURAT  </a>				</th>
				<!--
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" id="mail_box">
			...
			<tr bgcolor="<?php echo $background;?>"  onMouseOver="this.style.color='#666';this.style.cursor='pointer';"onmouseout="this.style.color='#000';" onclick="viewDetail('<?php echo $row['id'];?>','<?php echo $set_sts;?>');">
				<td width="18%">
					...
					<?php echo $pegawai->getField('jabatan',$row['mfrom'])."-".$pegawai->getField('nama',$row['mfrom']);?>
					</p>			  
			  </td>
				<td width="17%">
					<p style="padding-left:10px">
...
<?php
	...
	$url=$link.'&m=min&s='.$s.'&sts='.$sts.'&i='.$i;
	$id='id';
