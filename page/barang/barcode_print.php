...
if($x!=array()){	
	foreach($x as $data){
		for($i=0;$i<$data['qty'];$i++){
			$bdata=$data['id_brg'];
			$arraybar = array('encode'=>'CODE39','bdata'=>$bdata,'height'=>'30','scale'=>'2','bgcolor'=>'#FFFFFF','color'=>'#000000','file'=>'','type'=>'png');
		?>
			<div style="padding:5px;font-size:11px;width:430px;height:55px; margin-top:20px;float:left;background-color:white; text-align:center;">
				<?php
					foreach($arraybar as $keys=>$value){
						$qstr.=$keys."=".urlencode($value)."&";
					}
					echo "<img src='barcode_print.php?$qstr'>";
				?>
			</div>
		<?php
		}
