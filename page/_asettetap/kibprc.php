			...
			case 'KIBA':
				if(isset($_POST['id'])){
					...
					<select name="id_barang" id="id_barang">
						<?php 
						echo '<option selected="selected" value="'.$_POST['id'].'">'.$_POST['id'].'&nbsp;'.$dtkiba->getField('nama',$_POST['id']).'&nbsp;'.$dtkiba->getField('luas',$_POST['id']).'</option>';
						$x=$dtkiba->getAll();
						foreach($x as $row){
						echo '<option value="'.$row['id'].'">'.$row['id'].'&nbsp;'.$row['nama'].'&nbsp;'.$row['luas'].'</option>';
						}
						?>
					</select>
					<?php		
